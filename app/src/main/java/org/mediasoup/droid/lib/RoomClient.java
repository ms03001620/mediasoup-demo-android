...
public class RoomClient extends RoomMessageHandler {
    ...
    public RoomClient(Context context, RoomStore roomStore, RoomClientConfigData configData, RoomOptions options) {
        super(roomStore);
        ...
        mWorkHandler.post(() -> {
            try {
                if (mSendTransport != null) {
                    String iceParameters = mProtoo.syncRequest("restartIce", req -> jsonPut(req, "transportId", mSendTransport.getId()));
                    mSendTransport.restartIce(iceParameters);
                }
                if (mRecvTransport != null) {
                    String iceParameters = mProtoo.syncRequest("restartIce", req -> jsonPut(req, "transportId", mRecvTransport.getId()));
    ...
    @Async
    public void setConsumerPreferredLayers(String consumerId, String spatialLayer, String temporalLayer) {
        Logger.d(TAG, "setConsumerPreferredLayers()");
        // TODO: layer
    }
    ...
    public void changeDisplayName(String displayName) {
        ...
        mWorkHandler.post(() -> {
            try {
                mProtoo.syncRequest("changeDisplayName", req -> jsonPut(req, "displayName", displayName));
    ...
    private void joinImpl() {
        ...
        try {
            ...
            String joinResponse = mProtoo.syncRequest("join", req -> {
    ...
    private void createSendTransport() throws ProtooException, JSONException, MediasoupException {
        ...
        String res = mProtoo.syncRequest("createWebRtcTransport", (req -> {
    ...
    private SendTransport.Listener sendTransportListener = new SendTransport.Listener() {
        ...
        public void onConnect(Transport transport, String dtlsParameters) {
            ...
            mCompositeDisposable.add(mProtoo.request("connectWebRtcTransport", req -> {
                jsonPut(req, "transportId", transport.getId());
                jsonPut(req, "dtlsParameters", toJsonObject(dtlsParameters));
            }).subscribe(d -> Logger.d(listenerTAG, "connectWebRtcTransport res: " + d), t -> logError("connectWebRtcTransport for mSendTransport failed", t)));
        }
    ...
    private RecvTransport.Listener recvTransportListener = new RecvTransport.Listener() {
        ...
        public void onConnect(Transport transport, String dtlsParameters) {
            ...
            mCompositeDisposable.add(mProtoo.request("connectWebRtcTransport", req -> {
                jsonPut(req, "transportId", transport.getId());
                jsonPut(req, "dtlsParameters", toJsonObject(dtlsParameters));
            }).subscribe(d -> Logger.d(listenerTAG, "connectWebRtcTransport res: " + d), t -> logError("connectWebRtcTransport for mRecvTransport failed", t)));
        }
    ...
    private void onNewConsumer(Message.Request request, Protoo.ServerRequestHandler handler) {
        ...
        try {
            ...
            Consumer consumer = mRecvTransport.consume(c -> {
